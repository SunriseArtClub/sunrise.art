...
export const reduxSafeNewAuction = (auction: AuctionCreateEvent): IAuction => ({
  ...
  sunriseId: BigNumber.from(auction.sunriseId).toJSON(),
  settled: false,
});
...
export const reduxSafeBid = (bid: BidEvent): BidEvent => ({
  sunriseId: BigNumber.from(bid.sunriseId).toJSON(),
  sender: bid.sender,
...
) => BigNumber.from(a.sunriseId).eq(BigNumber.from(b.sunriseId));
