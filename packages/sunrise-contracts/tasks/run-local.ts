...
task(
  ...
  await contracts.SunriseAuctionHouse.instance
    .attach(contracts.SunriseAuctionHouseProxy.instance.address)
    .unpause({
      gasLimit: 1_000_000,
    });
  ...
  console.log(
    `sunrise contracts deployed to local node at http://localhost:8545 (Chain ID: ${chainId})`,
  );
  console.log(
    `Auction House Proxy address: ${contracts.SunriseAuctionHouseProxy.instance.address}`,
  );
  console.log(`Sunrise ERC721 address: ${contracts.SunriseToken.instance.address}`);
